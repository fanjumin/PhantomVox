...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  [PhantomVox LOGO]  StoryCut │ ProEdit │ Palette │ AudioForge │ EffectLab │ AI Agent │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  六个独立工作区 ─── 借鉴达芬奇核心理念，PhantomVox 自有命名体系                   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│  [AI Agent]  |  [💬聊天]  [🧠思考]  [🖼生图]  [🎬生视频]  [💻代码]  [📋导图]  [🔧矩阵]  │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ═══════════════════  【思维导图】 — 核心区域  ═══════════════════             │
│                                                                              │
...
│  ├───────────────────────────────────────────────────────────────────────┤ │
│  ├─ 图像生成 ──────────────────────────────────────────────────────────┐ │
│  │  [▼ 选择引擎]  Stable Diffusion XL │ DALL-E 3 │ Midjourney │ 本地     │ │
│  │  默认分辨率: 1024×1024 │ 质量: [标准/高清/极致]                       │ │
│  ├───────────────────────────────────────────────────────────────────────┤ │
│  ├─ 视频生成 ──────────────────────────────────────────────────────────┐ │
│  │  [▼ 选择引擎]  Runway Gen-4 │ Pika │ 本地模型                        │ │
│  │  最大时长: 10s │ 分辨率: 1080p │ 帧率: 24/30/60                       │ │
│  ├───────────────────────────────────────────────────────────────────────┤ │
│  ├─ 音频生成 ──────────────────────────────────────────────────────────┐ │
│  │  TTS: [Edge TTS / OpenAI TTS / CosyVoice / 本地]                     │ │
│  │  语音克隆: [CosyVoice / GPT-SoVITS / OpenVoice]                      │ │
│  │  音乐生成: [Suno / MusicGen / AudioCraft / 本地]                     │ │
│  ├───────────────────────────────────────────────────────────────────────┤ │
│  ├─ 图像修复 ──────────────────────────────────────────────────────────┐ │
│  │  [▼ 选择引擎]  GFPGAN (人脸) │ Real-ESRGAN (超分) │ CodeFormer        │ │
│  │  修复强度: [━━━●━━━]                                                  │ │
│  ├───────────────────────────────────────────────────────────────────────┤ │
│  └─ API 配置 ─────────────────────────────────────────────────────────┐ │
│     ┌────────────────────────────────────────────────────────────┐     │ │
...
| 模块 | 可配置项 |
|------|---------|
| **AI 聊天** | LLM 模型选择、上下文长度、温度、系统提示词模板 |
| **图像生成** | 引擎选择（云端/本地）、分辨率、质量 |
| **视频生成** | 引擎选择、最大时长、分辨率、帧率 |
| **音频生成** | TTS 引擎、克隆引擎、音乐生成引擎 |
| **图像修复** | 修复引擎（GFPGAN/ESRGAN/CodeFormer）、强度 |
| **API 配置** | 各厂商 API Key、本地模型路径、代理设置 |
...
> 所有六个工作区共享的全局菜单栏，位于窗口最顶部。PhantomVox 遵循专业软件惯例，但菜单项针对 AI 音视频创作做了定制。
...
| **AI 思维导图节点** | 执行/暂停/取消/编辑参数/回溯/展开分支/保存为模板 |
| **Agent 列表** | 激活/禁用/查看详情/分配任务/查看日志 |

---

## 十一、PhantomVox 模块命名对照
...
|----|------|------|
| 一 | 整体应用布局 | 6 个工作区标签 |
| 二 | StoryCut 页面 | 故事剪辑布局+AI |
| 三 | ProEdit 页面 | 专业编辑布局+AI |
| 四 | Palette 页面 | 色彩工坊布局+AI |
| 五 | AudioForge 页面 | 音频工坊布局+AI |
| 六 | EffectLab 页面 | 特效工坊布局+AI |
| 七 | AI Agent 页面 | 智能体控制中心 |
| 八 | 模型设置 | 全局 AI 模型配置 |
| 九 | FFmpeg 引擎层 | 音视频处理核心 |
| 十 | 全局菜单栏 | 8 个菜单+快捷键 |
...
| | └── 引擎模块注册机制 | — | ✅ 已完成 |
| **P1** | Agent 框架 + 思维导图 + 模型设置 | P0 | 从 0 搭建 |
| **P2** | ProEdit 页面 (时间线+属性面板+AI助手) | P1 | 核心编辑体验 |
| **P3** | AudioForge + AI 音频工坊 | P2 | **差异化功能** |
| | ├── TTS 引擎集成 (Edge/MOSS/F5/GPT-SoVITS) | | 分层接入: T1→CPU, T2→GPU |
| | ├── 声音克隆工作流 | | F5-TTS / GPT-SoVITS |
| | ├── 音乐生成面板 (Suno/MusicGen/Riffusion) | | 在线+本地双通道 |
| | └── 硬件感知: 自动提示用户升级 | | 用 P0 hardware 模块 |
| **P4** | Palette + EffectLab | P3 | 色彩+特效 |
| **P5** | StoryCut + AI Agent 完整版 | P4 | 全功能就绪 |
| **P6** | 代码生成引擎 + 视频生成 | P5 | 高级能力 |
| **P7** | Director Agent 端到端编排 | P6 | 一句话成片 |
